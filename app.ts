type Role = "Design" | "Engineering" | "Marketing" | "Product" | "Data";
...
const companyFilter = document.querySelector<HTMLSelectElement>("#company-filter");
...
const savedJobs = document.querySelector<HTMLDivElement>("#saved-jobs");
...
function createEmptyState(message: string): string {
...
companyFilter.addEventListener("change", (event) => {
  filters.company = (event.target as HTMLSelectElement).value;
  renderJobs();
});

roleFilter.addEventListener("change", (event) => {
  filters.role = (event.target as HTMLSelectElement).value;
  renderJobs();
});
...
jobList.addEventListener("click", (event) => {
  ...
  const button = target.closest<HTMLButtonElement>("[data-job-id]");
